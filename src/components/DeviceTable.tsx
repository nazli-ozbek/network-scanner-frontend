...
export default function DeviceTable({ ipRange, onUpdatingChange, overrideDevices, searchQuery }: DeviceTableProps) {
  ...
  return (
    <div className="p-4 mt-8 max-w-4xl mx-auto bg-white rounded shadow-md">
      ...
      {initialLoading ? (
        ...
        <div
          className={`max-h-96 overflow-auto rounded border transition-colors duration-500 ${
                    ...
                    {device.tags?.length ? (
                      <div className="flex flex-wrap gap-1 mb-2">
                        {device.tags.map((t) => (
                          <span
                            key={t}
                            className="px-2 py-0.5 rounded-full bg-gray-100 border flex items-center gap-1"
                          >
                            {t}
                            <button
